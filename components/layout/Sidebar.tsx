...
export default function Sidebar() {
  ...
  const { currentTrack, isPlaying, progress, duration, pause, resume, next, prev, seek, repeat, toggleRepeat, normalize, toggleNormalize } = usePlayer()
      ...
      <div style={{ padding: `10px ${isMobile ? '14px' : '56px 10px 14px'}`, borderTop: '0.5px solid #2a2a2a', flexShrink: 0 }}>
        <div style={{ background: '#1a1a1a', borderRadius: 8, padding: '10px 14px' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: 8 }}>
            ...
            <button onClick={toggleNormalize} title={normalize ? "Normalize: On (-14 LUFS)" : "Normalize: Off"} style={{ background: "none", border: "none", color: normalize ? "var(--accent)" : "#444", cursor: "pointer", padding: 4, display: "flex", alignItems: "center", gap: 3, fontSize: 10, fontFamily: "monospace" }}><Volume2 size={12} />{ normalize ? "N" : ""}</button>
