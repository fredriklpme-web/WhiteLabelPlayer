...
import { Play, Download, ListPlus, Pencil, Trash2, X } from 'lucide-react'
...
export default function TrackMenu({ track, playlists, position, onClose, onRefresh }: TrackMenuProps) {
  ...
  // Justera position så menyn inte går utanför skärmen
  const menuWidth = 200
      ...
      {renaming ? (
        <div style={{ padding: '10px 12px' }}>
          ...
          <div style={{ display: 'flex', gap: 6 }}>
            <button onClick={handleRename} style={{ flex: 1, background: '#111', border: 'none', color: '#fff', fontSize: 11, padding: '5px 0', borderRadius: 4, cursor: 'pointer' }}>Spara</button>
            <button onClick={() => setRenaming(false)} style={{ flex: 1, background: 'none', border: '0.5px solid #ddd', color: '#888', fontSize: 11, padding: '5px 0', borderRadius: 4, cursor: 'pointer' }}>Avbryt</button>
          </div>
        </div>
      ) : showPlaylists ? (
        <div>
          ...
          {playlists.length === 0 ? (
            ...
            <button key={pl.id} onClick={() => addToPlaylist(pl.id)} style={{ display: 'block', width: '100%', textAlign: 'left', padding: '9px 14px', fontSize: 12, color: '#333', background: 'none', border: 'none', cursor: 'pointer', borderBottom: '0.5px solid #f8f8f8' }}
